  ...
  const parsedRows = text
    ...
    .map((line) => {
      ...
      return {
        ...
        nakshatra:
          NAKSHATRA_FULL_NAMES[nakshatra as keyof typeof NAKSHATRA_FULL_NAMES],
        pada,
        ...
        navamsaCode: navamsa,
        hasKarmicDosha:
          KarmicNakshatras[rasi as keyof typeof KarmicNakshatras].includes(
            nakshatra
          ),
        karmicPlanet,
        karmicPlanetResults,
...
export const getMaitriByNakshatraAndPada = (
...
export const getMatchScores = (
  ...
  const scores: Array<{
    ...
    groom: string;
    score: number;
    scoreWithExceptions: number;
    remarks: string;
  }> = [];

  const getScoreByIndex = (
    index: number,
    brideItem: string,
    groomItem: string
  ) => {
    switch (index) {
      ...
      case 7: {
        ...
        let naadiRemarks = '';
        if (score === 0) {
          const { affectedStars, remarks } =
            NaadiNakshatraExceptions[brideItem];
          const brideStar = brideMaitri[2];
          const groomStar = groomMaitri[2];
          if (
            affectedStars.includes(brideStar) &&
            affectedStars.includes(groomStar)
          ) {
