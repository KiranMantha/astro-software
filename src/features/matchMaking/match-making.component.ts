...
export class MatchMakingComponent {
  ...
  totalMaxScore = computed(() =>
    this.ASTA_KOOTAMI.reduce((sum, item) => sum + item.maxScore, 0)
  );

  totalCurrentScore = computed(() =>
    this.scores().reduce((sum, item) => sum + (item.score || 0), 0)
  );
  ...
  handleSubmit() {
    ...
    const brideMaitri = getMaitriByNakshatraAndPada(
      parsedBride[2].nakshatraCode,
      parsedBride[2].pada
    );
    const groomMaitri = getMaitriByNakshatraAndPada(
      parsedGroom[2].nakshatraCode,
      parsedGroom[2].pada
    );
